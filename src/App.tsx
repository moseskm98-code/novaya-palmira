...
export function App({path}:{path:string}) {
  ...
  return <>
    ...
    {modal&&<Dialog title={modal==='image'?(imageView?.title||'Изображение'):modal==='video'?activeMedia.title:modal==='plan'?`${plan.title} · ${plan.area} м²`:modal==='quarter'?'Квартал, который можно увидеть':modal==='faq'?'Вопросы о покупке':modal==='privacy'?'Как используются данные':'Подтвердите обращение'} wide={modal==='image'||modal==='video'||modal==='plan'||modal==='quarter'} onClose={()=>setModal(null)}>
      {modal==='video'&&<><video ref={videoRef} className="dialog-video" src={activeMedia.src} poster={activeMedia.poster} controls playsInline preload="metadata" onError={()=>setVideoError(true)}/>{videoError&&<p role="alert">Не удалось воспроизвести видео. <a href={activeMedia.src} target="_blank" rel="noreferrer">Открыть ролик отдельно</a></p>}<p className="dialog-note">{activeMedia.caption}</p></>}
      {modal==='image'&&imageView&&<><img className="source-image" src={imageView.src} alt={imageView.alt}/><div className="dialog-actions"><a className="text-link" href={imageView.src} target="_blank" rel="noreferrer">Открыть в полном размере<ArrowRight size={18}/></a></div></>}
