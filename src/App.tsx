...
import { pages, findPage, plans, categories, faqs, address, phone, type Category } from './data';
import { siteBase, sitePath } from './sitePath';

import { ChapterNav, FamilySection, PowerSection, PowerDetails, Infrastructure, QuarterPlan, PurchaseSection, QualitySection, ManagementDetails, CompanyInfo, ResidentStories, NeighborhoodSummary, SpecializedDetails, type Media, type ImageView } from './ContentSections';

type ModalKind = 'image'|'video'|'plan'|'quarter'|'faq'|'privacy'|'draft'|null;
type Draft = {url:string;message:string};

function Brand({compact=false}:{compact?:boolean}) {
  return <a className={`brand ${compact?'brand-small':''}`} href={sitePath('/')} aria-label="Новая Пальмира — главная"><img src={sitePath('/brand/palmira-mark-gold.png')} width="62" height="58" alt=""/><span><strong>Новая Пальмира</strong><small>Жилой квартал</small></span></a>;
}
function DeveloperBrand(){return <a className="developer-brand" href={sitePath('/zastroishchik')} aria-label="Застройщик Мегаполис Групп"><img src={sitePath('/brand/megapolis-logo.png')} width="112" height="70" alt="Мегаполис Групп"/></a>;}
...
export function App({path}:{path:string}) {
  ...
  const [extraMedia,setExtraMedia]=useState<Media|null>(null);
  ...
  const activeMedia=extraMedia||{src:page.video,poster:page.poster,title:page.videoTitle.replace('\n',' '),caption:page.videoText};
  const openMedia=(media:Media)=>{setExtraMedia(media);setModal('video');};
  const openPageVideo=()=>{setExtraMedia(null);setModal('video');};
  const openImage=(image:ImageView)=>{setImageView(image);setModal('image');};
  ...
  async function submit(e:FormEvent<HTMLFormElement>){
    ...
    try {
      const payload={name:data.get('name'),phone:data.get('phone'),consent:data.get('consent')==='on',company:data.get('company'),request:request||requestText||'Просмотр квартала',page:page!.path,attribution};
      if(siteBase){
        const name=String(payload.name||'').trim().replace(/[\r\n]/g,' ');
        const tel=String(payload.phone||'').trim();
        let message=`Здравствуйте! Меня зовут ${name}. Интересует: ${payload.request}.\nТелефон: ${tel}.\nСтраница: ${page!.nav}.\nХочу согласовать просмотр и уточнить условия.`;
        const source=Object.entries(attribution).filter(([key,value])=>/^utm_(source|medium|campaign|content|term)$/.test(key)&&value).map(([key,value])=>`${key}: ${value.slice(0,200).replace(/[\r\n]/g,' ')}`).join('; ');
        if(source)message+=`\nИсточник: ${source}`;
        setDraft({message,url:`https://wa.me/${phone.whatsapp}?text=${encodeURIComponent(message)}`});setStatus('idle');setModal('draft');return;
      }
  ...
  return <>
    ...
    <main id="main">
      <section className={`hero ${home?'hero-home':'hero-detail'}`} aria-labelledby="hero-title">
        ...
        <div className="hero-content"><p className="eyebrow">{page.eyebrow}</p><h1 id="hero-title">{page.title}</h1><p className="hero-lead">{page.lead}</p><div className="hero-actions">{special?<a className="button" href="#object-selection">{page.cta}<ArrowRight size={19}/></a>:<a className="button" href="#plans">{page.cta}<ArrowRight size={19}/></a>}<button className="video-link" onClick={openPageVideo}><span className="play-outline"><Play size={14} weight="fill"/></span>{page.key==='management'?'Смотреть работу УК':page.key==='developer'?'Смотреть строительство':page.key==='commercial'?'Смотреть окружение':page.key==='electricity'?'Смотреть отзыв жительницы':'Смотреть квартал'}</button></div></div>
      ...
      <section className="video-section section-shell" aria-labelledby="video-title">
        <button className="video-preview" onClick={openPageVideo} aria-label={`Смотреть видео: ${page.videoTitle.replace('\n',' ')}`}><img src={sitePath(page.poster)} alt={page.key==='commercial'?'Инфраструктура квартала':'Реальная фотография Новой Пальмиры'} loading="lazy"/><span className="play-large"><Play size={38} weight="fill"/></span><span className="video-preview-caption">{page.key==='mansards'?'Обзор квартала':'Новая Пальмира · Видео'}</span></button>
        <div className="video-story"><h2 id="video-title">{page.videoTitle}</h2><p>{page.videoText}</p><span className="small-rule"/><button className="text-link" onClick={openPageVideo}>Смотреть видео<ArrowRight size={19}/></button></div>
      </section>
      {home&&<><FamilySection onMedia={openMedia}/><PowerSection onMedia={openMedia}/></>}
      ...
      {page.key==='electricity'&&<PowerDetails onRequest={startRequest}/>}
      {page.key==='management'&&<ManagementDetails onRequest={startRequest}/>}
      {special&&<SpecializedDetails kind={page.key as 'mansards'|'commercial'} onRequest={startRequest}/>}
      ...
      {home&&<><Infrastructure onMedia={openMedia}/><QuarterPlan onImage={openImage} onRequest={startRequest}/></>}
      {!home&&<NeighborhoodSummary pageKey={page.key}/>}
      <PurchaseSection onRequest={startRequest} selected={request.split('. Оплата:')[0]||requestText}/>
      {(home||page.key==='developer')&&<QualitySection onRequest={startRequest} developer={!home}/>}
      {page.key==='developer'&&<CompanyInfo onRequest={startRequest}/>}
      ...
      {(home||page.key==='developer')&&<ResidentStories onImage={openImage}/>}
    ...
    {modal&&<Dialog title={modal==='image'?(imageView?.title||'Изображение'):modal==='video'?activeMedia.title:modal==='plan'?`${plan.title} · ${plan.area} м²`:modal==='quarter'?'Квартал, который можно увидеть':modal==='faq'?'Вопросы о покупке':modal==='privacy'?'Как используются данные':'Подтвердите обращение'} wide={modal==='image'||modal==='video'||modal==='plan'||modal==='quarter'} onClose={()=>setModal(null)}>
      {modal==='video'&&<><video ref={videoRef} className="dialog-video" src={sitePath(activeMedia.src)} poster={sitePath(activeMedia.poster)} controls playsInline preload="metadata" onError={()=>setVideoError(true)}/>{videoError&&<p role="alert">Не удалось воспроизвести видео. <a href={sitePath(activeMedia.src)} target="_blank" rel="noreferrer">Открыть ролик отдельно</a></p>}<p className="dialog-note">{activeMedia.caption}</p></>}
      {modal==='image'&&imageView&&<><img className="source-image" src={sitePath(imageView.src)} alt={imageView.alt}/><div className="dialog-actions"><a className="text-link" href={sitePath(imageView.src)} target="_blank" rel="noreferrer">Открыть в полном размере<ArrowRight size={18}/></a></div></>}
      {modal==='plan'&&<><img className="dialog-plan" src={sitePath(plan.src)} alt={`Оригинальная планировка ${plan.area} м²`}/><div className="dialog-actions"><a className="text-link" href={sitePath(plan.src)} download={`Новая-Пальмира-${plan.area}.webp`}>Скачать планировку<ArrowRight size={18}/></a><button className="button" onClick={()=>startRequest(`${plan.title}, ${plan.area} м²`)}>Узнать условия<ArrowRight size={18}/></button></div></>}
      {modal==='quarter'&&<><p>Новая Пальмира — дома, дворы и привычные дела рядом. Начните знакомство с прогулки по кварталу.</p><img className="quarter-photo" src={sitePath('/images/courtyard.jpg')} alt="Реальный двор жилого квартала"/><div className="quarter-details"><div><h3>Приезжайте познакомиться</h3><p>{address}</p><a className="text-link" href={`https://yandex.ru/maps/?text=${encodeURIComponent(address+' Новая Пальмира')}`} target="_blank" rel="noreferrer">Открыть карту<ArrowRight size={18}/></a></div><div><h3>Всё необходимое рядом</h3><p>Прогулочные пространства, магазины и сервисы на первых этажах, общие зоны и управляющая компания.</p></div></div><details><summary>Посмотреть схему квартала</summary><img className="quarter-scheme" src={sitePath('/images/scheme.jpg')} alt="Схема корпусов Новой Пальмиры"/><p className="dialog-note">Схема из материалов проекта. Актуальное наличие и сроки уточняйте в офисе продаж.</p></details><button className="button" onClick={()=>startRequest('Личный просмотр квартала')}>Договориться о просмотре<ArrowRight size={18}/></button></>}
