...
const planRequest=(item:(typeof plans)[number])=>`${item.title}, ${item.area} м² — ${item.variant}`;
...
export function App({path}:{path:string}) {
  ...
  return <>
      ...
      <section id="plans" className="plans section-shell" aria-labelledby="plans-title">
        ...
        <div key={category} className="plan-track" role="tabpanel" id="plan-panel" aria-labelledby={`tab-${category}`} tabIndex={0} onScroll={e=>{
          ...
          {selection.map((item,index)=><div key={item.id} className={`plan-layout ${item.id===plan.id?'is-active':''}`}>
            <div className="plan-visual"><button className="plan-image-button" onClick={()=>{setPlanId(item.id);setModal('plan');}} aria-label={`Увеличить планировку ${item.area} м², ${item.variant}`}><img src={sitePath(item.src)} alt={`План: ${item.title}, ${item.area} м², ${item.variant}`} loading="lazy" draggable={false}/><span className="zoom-hint"><MagnifyingGlassPlus size={18}/>Увеличить</span></button></div>
            <div className="plan-info"><h3>{item.title}</h3><div className="plan-area">{item.area.split(',')[0]},<small className="plan-decimal">{item.area.split(',')[1]}</small><span>м²</span></div><p><strong className="plan-variant">{item.variant}</strong><br/>{item.description}</p><button className="button" onClick={()=>startRequest(planRequest(item))}>Узнать условия<ArrowRight size={18}/></button><div className="plan-pagination"><button className="icon-button" onClick={()=>selectPlan(-1)} aria-label="Предыдущая планировка"><ArrowLeft size={20}/></button><span>{String(index+1).padStart(2,'0')}<small> / {String(selection.length).padStart(2,'0')}</small></span><button className="icon-button" onClick={()=>selectPlan(1)} aria-label="Следующая планировка"><ArrowRight size={20}/></button></div></div>
          </div>)}
        </div>
        <p className="sr-only" role="status">Планировка {planIndex+1} из {selection.length}: {plan.title}, {plan.area} м², {plan.variant}</p>
      </section>
    ...
    {modal&&<Dialog title={modal==='video'?page.videoTitle.replace('\n',' '):modal==='plan'?`${plan.title} · ${plan.area} м² · ${plan.variant}`:modal==='quarter'?'Квартал, который можно увидеть':modal==='faq'?'Вопросы о покупке':modal==='privacy'?'Как используются данные':'Подтвердите обращение'} wide={modal==='video'||modal==='plan'||modal==='quarter'} onClose={()=>setModal(null)}>
      {modal==='video'&&<><video ref={videoRef} className="dialog-video" src={sitePath(page.video)} poster={sitePath(page.poster)} controls playsInline preload="metadata" onError={()=>setVideoError(true)}/>{videoError&&<p role="alert">Не удалось воспроизвести видео. <a href={sitePath(page.video)} target="_blank" rel="noreferrer">Открыть ролик отдельно</a></p>}<p className="dialog-note">{page.key==='mansards'?'Обзор квартала. Видео конкретной мансарды запрашивается отдельно.':page.videoText}</p></>}
      {modal==='plan'&&<><img className="dialog-plan" src={sitePath(plan.src)} alt={`Оригинальная планировка ${plan.area} м², ${plan.variant}`}/><div className="dialog-actions"><a className="text-link" href={sitePath(plan.src)} download={`Новая-Пальмира-${plan.id}.webp`}>Скачать планировку<ArrowRight size={18}/></a><button className="button" onClick={()=>startRequest(planRequest(plan))}>Узнать условия<ArrowRight size={18}/></button></div></>}
